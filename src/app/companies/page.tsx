import { companies } from "@/lib/company-config";
import { loadCompanyFromEngine } from "@/lib/load-company";
import { redirect } from "next/navigation";

export default async function CompaniesPage() {
  const { codeName } = await loadCompanyFromEngine();
  if (codeName) {
    redirect("/");
  }

  const uniqueCompanies = Object.entries(companies).filter(
    ([key]) => key !== "urban-bite"
  );
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 to-black flex items-center justify-center p-6">
      <div className="w-full max-w-7xl space-y-12">
        <div className="text-center space-y-3">
          <h1 className="text-5xl font-bold text-white">Choose Your Company</h1>
          <p className="text-gray-400">Select a company to continue</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {uniqueCompanies.map(([key, config]) => (
            <div
              key={key}
        ...
        <div className="text-center p-4 bg-red-500/10 border border-red-500/20 rounded-xl">
          <p className="text-sm text-red-400">
            ⚠️ Booking engine did not return a known{" "}
            <code className="px-2 py-1 bg-black/30 rounded">codeName</code> for
            this host. Check engine URL and ClientUrl.
          </p>
        </div>
      </div>
    </div>
