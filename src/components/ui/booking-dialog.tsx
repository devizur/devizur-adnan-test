...
import { nextStep, prevStep, resetBooking, addActivity, addPackage, addFood, setStep } from "@/store/bookingSlice";
...
export function BookingDialog({
  ...
  const cart = useCart();
  const { step, timeSlot, persons, selectedActivities, selectedPackages, selectedFoods } =
      ...
      >
        ...
        <div
          className={cn(
            ...
            step === 1 ? "min-h-0 flex flex-col overflow-hidden" : "overflow-y-auto px-6 py-5"
          )}
        >
        ...
        <AlertDialogFooter className="px-6 py-4 border-t border-gray-800 flex-row justify-between gap-3 flex-wrap bg-[#1a1a1a] shrink-0">
          ...
          <div className="flex items-center gap-2">
            {step === 1 && (
              <Button
                type="button"
                disabled={!canProceedStep1 || !hasSelectionStep1}
                onClick={handleNext}
